---
tags: ["Dev"]
title: "Go转Java必看, 跨越3大思维鸿沟"
linkTitle: "Go转Java起手篇, 跨越3大思维鸿沟"
weight: 31
description: >
...
Go里也有设计模式，不过更倾于使用函数、闭包和通道等语言特性来解决。先从直观语法上，总结有以下几种差异情况（只总结差异，不做优劣评价）：

> Go: 极简主义​ - 少即是多, Go中有很多隐含的写法规则，在Java里则需要编写显式的修饰符（关键字）声明，比如某结构（类）中，首字母大写，是会暴露一个字段，在java则需要显示的使用public。
...
> 语言只是工具，工程化才是本质。无论是Go的"显式控制"还是Java的"约定优于配置"，最终目标都是构建高质量、可维护、可演进的软件系统。成功的工程师不是精通某一种语言，而是理解不同语言背后的设计哲学，并能够根据项目需求、团队技能、业务场景做出明智的技术选择。

在云原生时代，工程化能力比语言语法更重要。掌握如何设计良好的API、如何管理依赖、如何组织代码结构、如何保证质量，这些能力在任何语言中都是相通的。